        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Failure-Predictive Morphology: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot morphology and learning. The motivating bet is: Predict which morphology-specific constraints will dominate policy failure.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For robot morphology and learning, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
\midrule
observed\_only\_wam & nominal & 0.522 & 0.478 \\
observed\_only\_wam & hidden\_mode\_shift & 0.177 & 0.823 \\
augmentation\_as\_data\_wam & nominal & 0.642 & 0.358 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.282 & 0.718 \\
scalar\_uncertainty\_wam & nominal & 0.647 & 0.353 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.352 & 0.647 \\
proposed\_branch\_mechanism & nominal & 0.752 & 0.248 \\
...
\end{tabular}
        ...
        \section{Conclusion}
        Failure-Predictive Morphology argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
